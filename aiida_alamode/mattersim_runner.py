...
"""
Replacement of pw.x: compute total energy, forces and stress of displaced structures
with an ASE calculator (MatterSim by default).

This script runs on the computer side (inside a scheduler job) and must not import aiida.

usage: alamode-mattersim job.json
...
CALCULATORS = {
    ...
    "sevennet-polar": ("sevenn.calculator", "SevenNetCalculator",
                       {"model": os.environ.get("SEVENNET_POLAR_MODEL",
                        ...
                        "device": "auto"}),
    "orb": ("aiida_alamode.mattersim_runner", "_orb_calculator", {"model": "orb_v3_conservative_inf_omat", "device": "auto"}),
