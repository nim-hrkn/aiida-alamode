...
"""forces of all the displaced structures with a force-calculator CalcJob (ASE calculator by default), and the DFSET."""
import numpy as np
...
class ForcesWorkChain(WorkChain):
    """forces of the displaced structures, split into njobs jobs of the forces CalcJob, and the DFSET for alm.

    forces_plugin selects the engine: 'alamode.forces_ase' (ASE calculator, MatterSim by default; the
    ``calculator`` / ``model`` / ``device`` inputs are passed through) or a VASP / QE forces CalcJob with
    the same ``arrays`` output.
    ...
    def define(cls, spec):
        super().define(spec)
        spec.input("code", valid_type=AbstractCode, help="the code of the forces CalcJob (e.g. alamode-ase-runner)")
        spec.input("forces_plugin", valid_type=Str, default=lambda: Str("alamode.forces_ase"),
                   help="entry point of the forces CalcJob")
        ...
        spec.input("calculator", valid_type=Dict, required=False,
                   help="ASE calculator as in the forces CalcJob (forces_plugin)")
        ...
        spec.exit_code(400, 'ERROR_CALCJOB_FAILED', message='a forces CalcJob failed.')

    def _builder(self, structures, indices=None):
        builder = CalculationFactory(self.inputs.forces_plugin.value).get_builder()
