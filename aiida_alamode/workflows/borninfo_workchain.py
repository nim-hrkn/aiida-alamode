...
@calcfunction
def bec_from_values(structure: StructureData, values: List) -> ArrayData:
    """Z* given by hand: one entry per atom of structure, each 1 (isotropic), 3 (diagonal) or 9 values."""
    rows = values.get_list()
    if len(rows) != len(structure.sites):
        raise ValueError(f"{len(rows)} Born charges for {len(structure.sites)} atoms")
    arrays = ArrayData()
    arrays.set_array("bec", np.array([dielectric_matrix(row) for row in rows]))
    arrays.base.attributes.set("source", "input")
    return arrays
...
def make_borninfo(structure: StructureData, born_effective_charges: ArrayData, dielectric_tensor: ArrayData) -> dict:
    ...
    summary = {"symbols": [s.kind_name for s in structure.sites],
               ...
               "bec_source": born_effective_charges.base.attributes.get("source", None),
...
class BornInfoWorkChain(WorkChain):
    ...
    ``born_charges`` (one entry per atom of the structure, each 1, 3 or 9 values, e.g. literature Z* of a
    material outside the training set of the models) replaces the Z* job.
    """
    ...
    def define(cls, spec):
        ...
        spec.input_namespace("bec", dynamic=True, required=False,
                             help="inputs of the Born-charge CalcJob (code, calculator, options, ...)")
        spec.input("born_charges", valid_type=List, required=False,
                   help="Z* given by hand instead of the Z* job: one entry per atom of the structure "
                        "(1 isotropic, 3 diagonal or 9 values each)")
        ...
        spec.outline(
            if_(cls.needs_bec_job)(cls.run_bec, cls.inspect_bec).else_(cls.set_bec_values),
        ...
        spec.exit_code(409, 'ERROR_NO_BEC', message='no Born charges: give the bec inputs or born_charges.')
    ...
    def needs_bec_job(self):
        return "born_charges" not in self.inputs

    def run_bec(self):
        if "bec" not in self.inputs or "code" not in self.inputs.bec:
    ...
    def inspect_bec(self):
        ...
        self.ctx.born_effective_charges = self.ctx.bec.outputs.born_effective_charges
        if "dielectric_tensor" in self.ctx.bec.outputs:
            self.ctx.dielectric = self.ctx.bec.outputs.dielectric_tensor

    def set_bec_values(self):
        self.ctx.born_effective_charges = bec_from_values(self.inputs.structure, self.inputs.born_charges)

    def needs_epsinf_job(self):
        if "dielectric" not in self.ctx and "dielectric" in self.inputs:
    ...
    def assemble(self):
        self.out("born_effective_charges", self.ctx.born_effective_charges)
        self.out("dielectric_tensor", self.ctx.dielectric)
        out = make_borninfo(self.inputs.structure, self.ctx.born_effective_charges, self.ctx.dielectric)
