...
class PhononCalculator(object):
    ...
    def _parse_dos(self):
        ...
        if self.dos is None:
            ...
            if os.path.exists(fname_dos):
                self._parse_dos_(fname_dos)

    def _parse_dos_(self, f):
        """load the content of the dos file.
        ...
        if self.temperatures is None:
            ...
            if os.path.exists(fname_thermo):
                with open(fname_thermo) as f:
                    self._parse_thermo_(f)

    def _parse_thermo_(self, handle):
        """load the content of the thermo file.
        ...
        if self._frequencies is None:
            if os.path.exists(fname_log):
                with open(fname_log, 'r') as f:
                    self._parse_frequencies_(f)

    def _parse_frequencies_(self, handle):
        """parse the log file to get phonon frequencies.
    ...
    def _plot_band(self):
        ...
        files = [fname_to_plot]
        self.plot_band_(files)

    def plot_band_(self, files):
        """plot band dispersion
        ...
        plt.clf()
        return os.path.join(self._cwd, '%s_band.pdf' % self._prefix)
    ...
    def _plot_dos(self):
        ...
        """
        self.plot_dos_(self.dos)

    def plot_dos_(self, dos):
        """plot DOS if self.dos is True.
        ...
        ax = plt.subplot(gs[0])
        ax.plot(dos[:, 0] * kayser_to_mev, dos[:, 1] / kayser_to_mev)
        ax.set_ylim(0)
        ...
        plt.clf()
        return os.path.join(self._cwd, '%s_dos.pdf' % self._prefix)
    ...
    def _plot_thermo(self):
        ...
        plt.clf()
        return os.path.join(self._cwd, '%s_thermo.pdf' % self._prefix)
    ...
    def _preprocess_data(self, files, unitname, normalize_xaxis):
        ...
        for file in files:
            if isinstance(file, SinglefileData):
                _filepath = file.list_object_names()
                with file.open(_filepath[0]) as _f:
                    data_tmp = np.loadtxt(_f, dtype=float)
            else:
    ...
    def _get_kpath_and_kval(self, file_in):
        ...
        elif isinstance(file_in, SinglefileData):
            _filepath = file_in.list_object_names()
            with file_in.open(_filepath[0]) as _f:
                kpath_list, kval_float = self._get_kpath_and_kval_(_f)
        else:
            raise TypeError(f'unknown file_in type, type={type(file_in)}')
