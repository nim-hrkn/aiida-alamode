...
- `docs/intro.html` (bilingual introduction, standalone) is generated by `docs/build_intro.py` (reads the MgO run); it embeds
  `example/run_v013/MgO/MgO_report.html` (figures as SVG: `--figure-format svg`) and the graphviz output of
  `verdi node graph generate 14077 --ancestor-depth 1 --descendant-depth 1`, with computer names replaced by `host`.
  When the report layout or the MgO example changes, rebuild it.
