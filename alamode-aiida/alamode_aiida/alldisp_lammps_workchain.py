...
from alamode_aiida.misc import zerofillStr
...
def _pack_retrieved_to_FolderData(calculations, object_name, cwd, prefix, 
    ...
    for _i, w in enumerate(calculations):
        ...
        filename = filename_template.replace("{prefix}", prefix).replace("{counter}",counter)
...
class alldisplaces_lammps_WorkChain(WorkChain):
    ...
    def define(cls, spec):
        super().define(spec)
        spec.input("code_string", valid_type=Str)
        spec.input("structures", valid_type=TrajectoryData)
        spec.input('potential', valid_type=LammpsPotential)
        ...
        spec.input('options',
                   valid_type=Dict, default=lambda: Dict(dict=cls._RESOURCE))
    ...
    def inspect_workchains(self):
        ...
        results = {}
        for _i, w in enumerate(calculations):
            results[f"label{_i}"] = w.get_outgoing().get_node_by_label('results').get_dict()
